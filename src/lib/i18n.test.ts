...
test("a missing locale falls back to Persian", () => {
  assert.equal(safeLocale(fd(null)), "fa");
  assert.equal(safeLocale(fd("")), "fa");
});
...
test("a value that would escape the site is refused", () => {
  ...
  assert.equal(safeLocale(fd("/evil.com")), "fa");
  assert.equal(safeLocale(fd("//evil.com")), "fa");
  assert.equal(safeLocale(fd("https://evil.com")), "fa");
  assert.equal(safeLocale(fd("../../etc")), "fa");
});

test("an unknown but harmless locale still falls back", () => {
  assert.equal(safeLocale(fd("de")), "fa");
});
