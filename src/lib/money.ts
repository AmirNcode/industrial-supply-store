...
/**
 * Prices are stored once, in USD cents. Persian display converts at render time
 * using a single hand-maintained rate — there is no live FX feed in v1, and
 * pretending otherwise in the UI would be misleading.
 */
export const USD_TO_TOMAN = Number(process.env.USD_TO_TOMAN ?? 110000);
 ...
 */
function toToman(cents: number): number {
  const raw = (cents / 100) * USD_TO_TOMAN;
  return Math.round(raw / 100) * 100;
}

export function formatPrice(cents: number, locale: Locale): string {
  if (locale === "fa") return `${tomanFmt.format(toToman(cents))} تومان`;
  return usdFmt.format(cents / 100);
}

/** Bare number, no currency word — for dense table columns with a unit header. */
export function formatPriceBare(cents: number, locale: Locale): string {
  if (locale === "fa") return tomanFmt.format(toToman(cents));
